...
`GameTextUseOverlayFont` sweeps ALL game text (legacy `Text`, `TMP_Text` with the original asset added as fallback, 3D `TextMesh`) onto the **game font** (`GameFontName`, decoupled from the overlay font since the Game UI tab; titles use `GameTextTitleWeight`, default heaviest; `GameStatsScale` separately sizes `*StatsText*` panels; `GameJudgementScale` separately sizes the hit-judgement popups (scrHitTextMesh) — both independent of the global game-text scale), caching originals (font, size, line spacing, style, best-fit max) for toggle-off restore. Every swap scales the size by the line-height/em ratio of original vs ours (clamped; `TextMesh` gets ×0.8 extra), **times a baked base ×0.6 times the `GameTextScale` slider** (base lowered 0.7→0.6 June 14 2026 — Pretendard ran large across the board), and widens leading by **a baked base ×1.5 times the `GameTextLineSpacing` slider** (bases hand-tuned for Pretendard June 2026, sliders centered at 1.0; Pretendard's glyphs fill more of the em box, so equal-metric leading reads cramped; legacy `Text`/`TextMesh` `lineSpacing` is a multiplier, TMP's is additive in font units ≈ em%). The Game UI tab has no on/off toggle (June 2026): the "Game font" selector carries a prepended "Game default" entry (BuildFontSelector's `defaultOption`/`onDefault` params) — picking it sets `GameTextUseOverlayFont=false` and hides the options block (`optionsHost`); picking any family enables the swap. The sliders call `RequestResize()`: a debounced (+15 frames, via `Tick`) full `Restore()`+`Apply()`, needed because `Apply` skips text already on our font. **Desired-font skip guard (June 13 2026):** the three Apply* methods now compute the bold decision + target font/style BEFORE the early-out, and skip only when the component already matches BOTH the target font and style. The old guard skipped on "font is one of ours" alone — but legacy `Text`/`TextMesh` bold is faux via `fontStyle` (font stays `_font`), so once a keycap was on `_font`+Bold or a world name on `_font`+Normal, bold-ness could never flip on a re-sweep (keycaps stuck bold, names stuck regular even after the InLevelSelect rule changed). **Re-stamp adoption (June 2026):** when a sweep finds an already-cached component carrying a font that isn't ours (`IsOurLegacyFont`/`IsOurTmpFont` — regular, bold, and every element-weight font), the game re-stamped it (Start()-time localization runs after the scene-entry sweep) — the cached restore Font (and TextMesh material) is updated to the re-stamped one, otherwise switching back to Game default restored pre-localization Latin-only fonts and Korean text rendered as tofu (title-screen tip/credits). **All sizes derive from the cached ORIGINAL state, never current values** — the game re-assigns its own localized fonts to surviving components on rewind, defeating the `font == ours` skip; computing from current values compounded the scale once per attempt. **Title bolding:** a component is bold-swapped when `scrHUDText.isTitle` is set, OR its cached ORIGINAL `fontStyle` requests bold, OR its ORIGINAL font's name looks bold (bold/black/heavy/-bd), OR `LevelSelectBold` says so while the ACTIVE scene is `scnLevelSelect` (June 13 2026: gate on the active scene via `InLevelSelect()`, NOT the component's own scene — the title/menu world content, floors, world names, keycaps, credits, are parented under DontDestroyOnLoad so their own scene isn't scnLevelSelect). In level select that verdict is AUTHORITATIVE — the caller branches and ignores `NameLooksBold`/style there, which otherwise mis-bold the single-letter keycaps. `LevelSelectBold` bolds everything except the `NewsSign` hierarchy, the press-to-start hint cluster (any ancestor GameObject named `Hit Space` — holds the cycling `numberKeys`/`GameComplete*` tips), `*StatsText*` panels, and single-glyph content (keycaps); the credits block is caught separately by `IsTitle` (scrCreditsText). IMPORTANT: the portal labels (계속/보정/레벨 에디터/커스텀 레벨) AND the visible `by 7th Beat Games` subtitle are all `scrTextChanger` components — so the tip cycler can NOT be excluded by component type (that suppressed all of them); the `Hit Space` ancestor check is what isolates the actual hints (verified June 13 2026 via desktopText keys: hints are `levelSelect.numberKeys`/`GameCompleteFullPure` under `Phase 0/Text/Hit Space`, labels are `levelSelect.continue`/`calibration`/`levelEditor`/`customLevels`, subtitle is `levelSelect.subtitle`). The bold decision for all three text systems funnels through `ShouldBold(c, text, styleBold, origFontName)`: `ForceRegular`→never, `IsTitle`→always, else per active scene — `scnLevelSelect`→`LevelSelectBold` (authoritative, whole scene), `scnCLS`→`ClsBold` (custom-level select: bold ONLY the chrome — the screen `title`, `WorldNameCanvas` portal labels, the selected level's title `scnCLS.instance.portalName`, the difficulty name `Difficulty Container/.../txtValue` ("엄격"), and the `Loading` text "불러오는 중" — NOT the body copy / level descriptions / help text / txtDescription), else the per-component heuristic. `ForceRegular` overrides every bold signal for `scrLetterPress` keycap letters AND `scrBestMultiplierText` speed-trial badges ("1.5배") GLOBALLY — the in-level multi-key indicators use a bold source font and rendered Black. Auto-sizing TMP (level descriptions in scnCLS — `enableAutoSizing`) ALSO ignores `fontSize`: it fits text between `fontSizeMin/Max`, so short text balloons to Max — `ApplyTmp` scales the bounds (`fontSizeMin/Max`, cached/restored) by the same factor instead (June 14 2026; the RUSH E description rendered huge + clipped). Legacy `Text` with `resizeTextForBestFit` ignores `fontSize` — `ApplyText` scales `resizeTextMaxSize` by the same factor (cached/restored), which is also how the `Continue/LastLevel` ("8-X Jungle City") ×0.6 special case takes effect. Resolved in `ApplySelectedFont` to the family's heaviest weight (literal "Bold" as fallback). HOW bold renders differs by system: the bundled Black **legacy `Font` asset silently renders as regular** (dynamic-font name fallback), so legacy `Text`/`TextMesh` keep the regular font and get engine faux bold via `fontStyle` (original style cached/restored); TMP gets the real Black TMP asset (proven by the overlay combo) with the faux Bold flag stripped to avoid double-bolding. **Per-element weights (Game UI tab → Element weights):** `Settings.GameUiTextWeights` (`Key`/`Weight`, ""=Auto) overrides the bold/regular heuristic for the seven text-bearing GameUiLayout targets plus the synthetic `judgement` key (pooled scrHitTextMesh popups, world-space `TMPro.TextMeshPro`/`TMP_Text` — lookup gated to the TMP path and uses `GetComponentInParent<scrHitTextMesh>(true)`: the no-arg overload skips inactive GameObjects and the pooled popups are inactive at sweep/Show-prefix time, which silently broke the weight) and the `levelname` key (consumed by MainClass, not here); `GameUiLayout.OwnerKey(c)` maps a component to its target via frame-cached `IsChildOf` checks, `MainClass` hands GameFontApplier a weight→FontEntry table for the game family (`SetElementWeights`), and the three Apply* skip-guards compare against the per-element font so weight changes re-apply. Explicit weights render with faux bold stripped. Sweeps log "GameFont: sweep bold-swapped N" (debug) when the count changes. **Opt-in sweep diagnostics:** set `GameFontApplier.DiagEnabled = true` to dump, once per sweep (capped at 16 lines, `GameFontDiag` prefix), each matching text's path/scene/newsSign/textChanger/desktopText/stats/hint/title/lsBold/applied-font+style — `DiagFilter` (substring list; empty = all <40-char texts) controls what's logged. Kept in place after it pinned the level-select label components. `NewsSign.ShowNews` is postfixed with `ApplyTo` — the title-screen news sign fills its text only when the async news fetch lands, after the scene sweep. **Editor exemption:** `Text`/`TMP_Text` whose GameObject lives in the `scnEditor` scene get metric normalization only — no `GameTextScale`, no leading change — the editor's hand-fitted form panels break under resizing (and some editor labels auto-fit, so a global shrink lands unevenly). `TextMesh` is resized everywhere. Sweeps re-run on scene change / level start / delayed state-change ticks; pooled judgement popups repaint per-spawn via the `scrHitTextMesh.Show` prefix; the custom-level select re-sweeps after each `scnCLS.DisplayLevel` (navigating between levels re-stamps the default font on the portal info and fires no other sweep, so only the selected-at-refresh level kept the overlay font); the pause/settings menu re-sweeps on `PauseMenu.Show` / `ShowSettingsMenu` (shown over gameplay with no scene change — previously only picked up the font after a later sweep). **Pause-menu bolding:** `ShouldBold` bolds everything under a `PauseMenu` ancestor EXCEPT the `SettingsMenu` child subtree, which keeps its designed weight. The `SettingsMenu` exclusion is checked FIRST (before IsTitle/scene rules) so it holds everywhere — e.g. opening Settings from the main menu, where `LevelSelectBold`'s whole-scene bold would otherwise catch it. `RDString.SetLocalizedFont` (Text/TMP_Text/TextMesh overloads) is postfixed → `GameFontApplier.OnLocalizedFontSet` re-applies our font immediately (the language-selector previews stamp each language's own font over our swap; this keeps ours — a script Pretendard lacks will tofu, acceptable per request). **Sweep scoping (June 2026 — fixed lag spikes at start/death/retry):** full `FindObjectsByType`×3 scene scans (expensive on large custom maps) run only on scene change / first level entry / toggle-resize, and are frame-deduped (`_lastSweepFrame`; `SetFonts` bypasses — font identity changed). Everything else — the +2/+30 state-change ticks and `Play(isRestart=true)` retries — uses `ReapplyHud()` (EXCEPT in scnLevelSelect, where state-change ticks sweep fully via `StateSweep()`: portal labels / world names activate late as you approach, live outside any canvas, and the menu scene is small), a `GetComponentsInChildren` sweep of just `scrUIController.canvas`, which is where all mid-scene text spawns/re-stamps happen (death %, results, congrats, rewind re-localization). **But scene ENTRY needs delayed FULL sweeps too** (`RequestFullSweepSoon`, +2/+30, fired from `OnActiveSceneChanged` and `SetFonts`): localization assigns fonts in scene objects' `Start()`, one frame after `sceneLoaded`, stomping the immediate sweep — on cold launch the title screen stayed vanilla until the toggle was cycled (previously masked by the then-unscoped state-change sweeps). Known gap: world-space win-screen extras outside the canvas (e.g. event endscreen lanterns) are no longer caught by state-change sweeps. `StopMod` calls `GameFontApplier.RestoreAll()` — without it, a hot-reloaded assembly (fresh Font instances, empty caches) re-caches our scaled text as "original" and compounds across deploys.
...
- **Wrapper transforms** — every `scrUIController` element (death `txtPercent`, `txtCongrats`, `txtAllStrictClear`, `txtResults`, `txtPressToStart`, `txtCountdown`, `difficultyContainer`, `modifiersContainer`, `pauseButton`; keyed by field name, all public — plus the autoplay label, a `scrShowIfDebug` with no static accessor, found by type with a frame-throttled `FindObjectsByType` and cached) gets a full-stretch `BismuthGameUiWrap_<key>` RectTransform inserted above it. The wrapper's rect equals the parent's, so the element's anchors/anchoredPosition keep their exact meaning, and everything the game does to the element (difficulty show/minimize `DOAnchorPosX` tweens, text rewrites) happens *inside* the wrapper and never fights our offset/scale. Scale pivots on the element's measured center (recomputed each apply), so scaling doesn't slide elements toward screen center. Offsets are stored in `Settings.GameUiOverrides` (`List<GameUiOverride>`: `Key`/`OffX`/`OffY`/`Scale`, parent-canvas units). Fresh installs get a curated default layout (`MakeGameUiDefaults`, re-baked June 12 2026 from the author's setup: presstostart/congrats/countdown/percent/results/strictclear/autoplay repositioned + scaled) AND curated per-element text weights (`MakeGameUiWeightDefaults`: presstostart Thin, congrats Black, countdown/strictclear Bold, results/autoplay Light, judgement Black, levelname Medium), both seeded ONCE in `EnsureDefaults` behind the `GameUiDefaultsSeeded` flag; `GameTextUseOverlayFont` now defaults TRUE (game-font swap on out of the box, Pretendard-Regular family). Reset semantics: right-click on a handle → `ResetToDefault` (the Bismuth default when the key has one, else vanilla); Game UI tab buttons "Reset layout to Bismuth defaults" (`ResetAllToDefaults`) and "Reset layout to game defaults" (`ResetAllToGame`, removes every override + meter restore) — an empty list is a legitimate "all reset to vanilla" state, so emptiness alone must not re-seed (unlike the KV presets).
...
`GameUiEditor` mirrors `LocationEditor` (own SSO canvas at 31000, mutual exclusion between the two editors) and reuses `LocHandle`, which gained optional `GetScale`/`SetScale` (corner `ScaleGrip`s — uniform scale from the pointer-to-center distance ratio — plus scroll wheel), `ResetTarget` (right-click), `ShowInactive` (dimmed handle at the element's last layout position) and `TightBounds` (size to the union of visible child Graphics instead of the target rect — the error meter wrapper extends well below its drawn content). A `HandleSorter` on the editor canvas re-orders the handle sibling block by area (largest first) each frame, so smaller handles render and raycast above the screen-blanketing congrats/results boxes. The autoplay `scrShowIfDebug` lookup prefers an **active** instance (several carry a `Text`; caching an inactive one pinned the handle to a dead object) and debug-logs each search outcome. While the editor is open it force-shows the game targets: the FULL ancestor chain up to the canvas is activated (own-GO-only left invisible handles — win/fail screen containers are inactive), CanvasGroup/text alphas < 0.05 lifted, empty `Text`s filled with Korean sample strings per key — all recorded (four lists) and restored in reverse on Close. The `results` sample is special: the real screen is a multi-line colored breakdown, so `ResultsSample` invokes the game's own private `DetailedResults.GenerateResults(players[0].marginTracker)` via reflection (localized labels + authentic `<color>` values; falls back to the static placeholder if anything is null/throws). Bismuth's own overlay is NOT shown (`Overlay.EditMode` untouched — user preference). `txtLevelName` is deliberately NOT a target: Bismuth already owns its transform (`LevelNameScale`/`LevelNameY`).
...
UMMCompat doesn't run UMM's own updater, so the mod updates itself. `UpdateChecker.Begin(modEntry)` (called from `TryEagerInit`) fetches `Repository.json` (the URL from Info.json's `Repository` field, kept current by `release.sh`); a newer `Releases[0].Version` shows `UpdatePopup`: **Update now (requires restart)** / **Manual update** (opens the releases page, derived from `DownloadUrl`) / **Later**. "Update now" downloads the release zip and extracts the `Bismuth/` payload over every existing install dir — running copy last, dll last within each (UMM watches it for hot reload). Persistent data survives by construction: the zip carries only the payload and nothing is deleted; `MainClass.PersistNow()` flushes settings/counts first. On success the primary button becomes **Close**.

**Dual-install handling:** native UMM loads from `<game>/Mods/Bismuth`, MelonLoader+UMMCompat from `<game>/UMMMods/Bismuth`. When both exist (and `Settings.IgnoreDuplicateInstall` is unset), `DuplicateInstallPopup` asks which loader is in use and offers deleting the unused copy — deleting the *running* copy first flushes and carries `Settings.xml`/`keycounts.txt`/`BismuthAttempts.txt` to the kept dir. The version check runs after the prompt resolves.
...
`Canvas.sortingOrder` wraps beyond 32767 (35000 became −30536 and rendered *under* the settings panel). Layering: LocationEditor 31000 < settings panel 32000 < LogViewer 32500 < UpdatePopup 32600 < DuplicateInstallPopup 32601. Never exceed 32767.
